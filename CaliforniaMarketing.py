"""
Allure Cross-Browser Selenium Tests (Chrome + Firefox + Edge)

How to run and generate Allure report:

1) Run tests at pycharm's terminal command line and collect Allure results:
   py.test --alluredir=./AllureReports ./CaliforniaMarketing.py

2) View the report (choose one option):

   2.1) Serve report (opens in browser):
        allure serve ./AllureReports

        If you need to stop the report server:
        Press Control + C in the terminal.

   2.2) If option 2.1 is not working:
        First stop option 2.1 using Control + C,
        then generate a static report:
        allure generate --clean ./AllureReports

Both options (2.1 and 2.2) work with this test suite.
"""
